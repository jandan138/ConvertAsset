# Superpowers Process Artifacts

This directory is an internal process exception to the main documentation taxonomy.

ConvertAsset's first-class documentation uses:

- `docs/design/`
- `docs/operations/`
- `docs/records/`
- `docs/reference/`
- `archive/`

`docs/superpowers/` stores design specs and implementation plans produced by the
superpowers workflow so later agents can audit the decisions that led to a
large documentation or implementation change.

## Current Artifacts

- [Genesis-style documentation refactor design](specs/2026-05-13-docs-genesis-style-refactor-design.md)
- [Genesis-style documentation refactor implementation plan](plans/2026-05-13-docs-genesis-style-refactor.md)
- [Paper ACL-27 layout design](specs/2026-05-20-paper-acl27-layout-design.md)
- [Paper ACL-27 layout implementation plan](plans/2026-05-20-paper-acl27-layout.md)
- [GRScenes target resolver implementation plan](plans/2026-05-20-grscenes-target-resolver.md)
- [GRScenes render manifest implementation plan](plans/2026-05-20-grscenes-render-manifest.md)
- [GRScenes scratch materialization implementation plan](plans/2026-05-20-grscenes-scratch-materialization.md)
- [GRScenes reference closure planner implementation plan](plans/2026-05-21-grscenes-reference-closure-plan.md)
- [GRScenes material dependency closure planner implementation plan](plans/2026-05-21-grscenes-material-dependency-closure-plan.md)
- [GRScenes targeted materialization implementation plan](plans/2026-05-21-grscenes-targeted-materialization.md)
- [GRScenes full no-MDL scratch planner implementation plan](plans/2026-05-21-grscenes-full-nomdl-scratch-plan.md)
- [Local Isaac render-single design](specs/2026-05-20-local-isaac-render-single-design.md)
- [Local Isaac render-single implementation plan](plans/2026-05-20-local-isaac-render-single.md)
