...
| `mdl_runtime_closure.py` | AAN-11 follow-up：MDL transitive import/using graph、MDL 内部 texture 依赖、package-local mirror/rewrite provenance、material binding scope 和 runtime material compiler evidence |
...
| `AAN-11` | Material Runtime Closure | Phase 1 closeout 后的材料 runtime follow-up | MDL transitive dependency graph、MDL 内部 texture mirror、必要 rewrite provenance、material binding scope、runtime compiler/readback evidence | DryingBox 补齐当前暴露的 helper MDL 和 texture 缺口；required material runtime stderr 无 MDLC/module/texture/shader-node error；front/door-facing 和 orbit render 无红色/error fallback；package escape scan clean；MuffleFurnace 覆盖 MDL 内部 texture；Beaker_01 覆盖空 texture uniform 不误报；不新增 DryingBox path/name 特例 | required material helper/texture unresolved；source tree 被修改；rewrite 无 hash/provenance；required shader node failed；binding scope 无法追溯；AAN-06 render pass 被误当作 material parity；下游仍需手工修 MDL/texture | Post-MVP follow-up |
| `AAN-12` | Batch Admission | LabUtopia USD 批量准入实验 | 20-50 个 LabUtopia USD 资产的 ready / blocked / waiver / unsupported 统计和代表性 evidence | 使用 public CLI 和统一 manifest schema；报告 failure mode 分布；不手工补路径；能指导下一轮 profile/adapter 优先级 | 只挑已知成功资产；失败不落 manifest；统计口径绕过 waiver/blocker；把 unsupported 当 ready | Post-MVP |
...
| MDL transitive module graph | every non-native import/using reachable from required material MDLs resolves inside package or approved runtime roots | unbound/background-only helper warning with scoped claim boundary | required material helper module unresolved, ambiguous same-name module hash, or package escape |
| MDL internal texture closure | every non-empty `texture_2d("...")` literal used by required material MDLs resolves package-relative with hash | UDIM/template pattern recorded but outside required material scope | required material texture missing, case mismatch, remote/absolute path without mirror, or unresolved UDIM set |
| Material binding scope | required render meshes have direct or inherited material binding and binding strength is recorded | background or decorative material binding ambiguous but outside required prim scope | required prim has no effective binding, unintended override hides intended material, or binding cannot be traced |
| Runtime material compiler | zero MDLC / failed shader node / missing texture errors for materials bound to required prims | unrelated renderer/deprecation warning or background material warning | required material compiler error, `Failed to create MDL shade node`, invalid USD_MDL, or red/error fallback |
...
### AAN-11 material runtime closure follow-up

AAN-11 是 Phase 1 closeout 后的明确编号 follow-up，不重开 `AAN-04` 或 `AAN-06`。
它吸收之前讨论中的 `AAN-04R` 和 `AAN-06R`：静态上补齐 MDL 内部 transitive
dependency，运行时上证明 required prim 绑定材质没有 MDLC / shader node / texture
load 错误。它在未来流水线的实际执行位置是 `AAN-04 Material Closure` 之后、
`AAN-06 Runtime Smoke` 之前或随 AAN-06 runtime 证据一起评估；对外编号仍使用
`AAN-11-material-runtime-closure`，避免把已收口的 MVP gate 重新打开。

AAN-11 的 claim boundary：

- 可以声明：required target surface 的 MDL/material runtime dependency closure 已完成，
  runtime material compiler 对 required material 干净，关键视角 render 没有红色/error fallback
  证据。
- 不能声明：full material parity、pixel-perfect render parity、任意 MDL 语义无损、所有背景
  材质都完全等价。

AAN-11 的 route policy 是 MDL-first，而不是 missing-first fallback。静态扫描看到
`.mdl` 或 texture 字符串缺失时，Normalizer 先判断它是不是 active dependency、是否绑定
required surface、以及 Isaac 4.1 runtime 是否真的把目标材质渲坏。只有 required-scope
证据失败，才进入 PreviewSurface / no-MDL fallback。换句话说：缺依赖报告是 triage
input，不是自动降级判决。

真实渲染正常时，Normalizer 也不能反过来跳过闭包证据。视觉 PASS 的含义是“不应仅凭
raw missing list 整包降级”，而不是“native MDL 自动干净”。保留 native MDL 至少需要：
active dependency trace、required material binding trace、required-scope runtime compiler
counters、以及 target multi-view visual evidence 同时成立。背景材质缺口可以进入 scoped
warning；required target surface 的缺口仍然 blocked。

Manifest 新增 `material_runtime_closure`。每条记录至少包含：

- `status`、`claim_level`、`full_material_parity_claimed=false`；
- `route_decision`：`source_mdl_native`、`source_mdl_mirrored`、
  `targeted_preview_fallback`、`package_preview_fallback` 或 `blocked`；
- `source_material_completeness`：source package 是否缺 helper MDL / MDL-internal
  texture，以及缺口是否 active；
- `target_runtime_viability`：required/background scope 下 runtime compiler 和 render
  是否可接受；
- `root_mdl_assets`：来自 `material_closure[*].source_mdl_assets` 和递归
  `deps/mdl/**/*.mdl`；
- `imported_mdl_modules`、`native_runtime_modules`、`unresolved_imports`；
- `mdl_texture_assets`、`unresolved_textures`；
- `mirror_actions`：helper MDL / MDL 内部 texture 的 source-to-package copy 证据；
- `rewrite_actions`：before/after sha256、规则名、影响行数、风险说明；
- `binding_scope`：required prim、effective material、direct/inherited binding、binding
  strength、unknown required render prim、non-render required prim；
- `runtime_compiler`：stderr/stdout counters、MDLC error、failed shader node、missing texture；
- `view_evidence`：front/door-facing、orbit_3q、transparent rim/outline 等 render artifact 和指标。

默认策略是先 mirror，不轻易 rewrite MDL 文本。只有 flatten 冲突、package escape、
runtime 必要且已知安全的 import 修复才 rewrite。旧项目中 KooPbr 的
`import ::KooPbr::...` 到相对 import 的修复只能作为“特定问题有证据后可 rewrite”的经验，
不能泛化成所有 MDL 的默认规则。

### UsdPreviewSurface fallback 允许条件

- 缺失 MDL / texture 字符串本身不构成 fallback 条件；必须先完成 active dependency、
  binding scope、runtime compiler 和 render evidence 分类。
...
- target profile 不要求 MDL native fidelity，或 remote/local MDL mirror 不可用但 benchmark
  允许 PreviewSurface。
- required material 在 native MDL 下无法合法 mirror/resolve，或 Isaac 4.1 对 required
  material 报 MDLC、missing module/texture、failed shader node、红/粉/error fallback。
...
13. `AAN-10 MJCF Scout`：先把 AutoBio/MJCF 路线落成 source inventory 和 semantic gap
    report，不把 scout 当成 EBench-ready package。
14. `AAN-11 Material Runtime Closure`：作为 Phase 1 closeout 后的明确 follow-up，补齐 MDL
    transitive dependency、MDL 内部 texture、binding scope、runtime compiler 和多视角 material
    evidence。它解决 DryingBox/MuffleFurnace 这类 runtime MDLC 报错和材质偏红问题，不重开
    AAN-04/AAN-06 历史 milestone。
15. `AAN-12+ Post-MVP Expansion`：进入 batch admission、consumer SDK、多 source adapter、
    多 target profile、Genesis / MuJoCo / EOS adapter 等路线；先做 source adapter 和 semantic
    gap report，不承诺一口气完成 official reproduction。
...
看起来 ready，但 runtime 中 MDL 依赖没有闭环导致材质偏红、shader node 创建失败或 texture
丢失”的问题。

| Step | Name | Scope | Acceptance criteria | Fail / blocked criteria |
|---|---|---|---|---|
| `AAN-11.1` | MDL Runtime Graph Inventory | 从 `material_closure[*].source_mdl_assets` 和 package `deps/mdl/**/*.mdl` 建立 MDL root set，扫描 `import`、`using` 和非空 `texture_2d("...")` 字面量 | 递归 graph 有 visited set；标准库 `df/base/state/tex/anno/math` 等归为 `native_runtime_module`；注释和空 texture uniform 不误报；`.::templates::mdl_0001::*` 这类嵌套相对模块能解析到 `deps/mdl/templates/mdl_0001.mdl` | required material 的 import/texture 未进入 graph；用全盘 find 代替确定 source root；循环 import 导致不收敛 |
| `AAN-11.2` | Transitive Mirror | 优先按 owning MDL 的相对路径语义和 AAN-04 root MDL `resolved_path` 复制 helper MDL 和 texture 到 package；Isaac runtime 自带 module 只能通过 approved runtime root 解析 | 每个 dependency 分类为 `mirrored`、`native_runtime_module`、`waived` 或 `blocked`；source path、package path、sha256 和 `mirror_actions` 可追溯；approved runtime module 记录 `runtime_root`、`runtime_path`、`runtime_module`、`runtime_sha256`；source-context mirror 可覆盖 DryingBox texture 缺口、MuffleFurnace nested template helper/texture 和 Beaker_01 glass helper | `local_missing` 进入 pass manifest；同名不同 hash 未阻断；required helper/texture 只能靠外部 source tree 解析；把未证明的 Isaac runtime module 直接 hard-code native |
| `AAN-11.3` | Safe Rewrite | 只在 mirror 不能保持语义、package escape 或已知安全修复时 rewrite MDL 文本 | rewrite action 记录 rule、before/after sha256、affected line count、risk；rewrite 后 package scan clean；不修改 source tree | 默认批量改 import；无 provenance；把 KooPbr 专用规则泛化；source tree 被写入 |
| `AAN-11.4` | Binding Scope Audit | 建立 required prim/render mesh 到 effective material 的 direct/inherited binding 图 | required asset root、manipulated body 或 render mesh 的 material binding 可追溯；binding strength 和覆盖关系记录；背景材质问题不污染 required scope | required prim 无有效材质；意外 override 隐藏 intended material；无法判断 compiler error 是否绑定 required prim |
| `AAN-11.5` | Runtime Compiler Gate | 在 AAN-06 cold load/render 后解析 stdout/stderr 和 runtime report | 记录 `stderr_bytes`、`[Error]`、`[Warning]`、`MDLC`、`USD_MDL`、`omni.hydra`、`rtx.mdltranslator` counters；required material 的 MDLC/module/texture/shader-node error 为 0；first N exemplar lines 带 line hash | required material 出现 `could not find module`、missing texture、`Failed to create MDL shade node`、`USD_MDL ... Invalid`、红色/error fallback |
| `AAN-11.6` | Multi-view Material Evidence | 在 orbit_3q_smoke 基础上增加 front/door-facing 或 transparent beaker view | 每个 view 记录 camera pose、target prim、bbox source、render hash、mean/foreground RGB、non-background ratio、bbox ratio；DryingBox 有 front/door-facing evidence；Beaker_01 有 rim/outline/opacity 可见性 evidence | PNG 存在但 all-background；expected red/transparent evidence 与 contract 冲突；多视角 hash 异常一致且相机应变化 |
| `AAN-11.7` | Replication And Negative Gate | DryingBox 首验后复制到 MuffleFurnace 和 Beaker_01，并保留一个缺 helper/texture 负例 | DryingBox 修复 helper/texture；MuffleFurnace 捕获 MDL 内部 texture，例如 `folder_0000/image_0000.jpg`；Beaker_01 空 texture uniform 不误报；负例稳定 blocked；无 DryingBox path/name 特例 | 只能修 DryingBox；复制资产需人工改 USD/MDL；负例误判 ready；runtime evidence 缺失 |
| `AAN-11.8` | MDL-Preserving Candidate Gate | 当静态 material closure 有缺口但可能是 registry residue、background-only 或非 required scope 时，允许 sidecar runtime/render candidate 在 fallback 前运行 | candidate 记录 route、root USD、MDL search paths、stdout/stderr、render hashes、visual verdict；raw missing refs 必须先 join 到 active dependency、binding scope、runtime log 和 visual evidence；required-scope runtime/render 通过时保留 MDL，背景问题进入 scoped warning；当前 pipeline 静态 gate 会阻止 runtime 的限制被显式记录 | 把 raw missing list 直接转成 no-MDL；candidate 依赖外部 source tree 才能最终消费；视觉 PASS 被用来覆盖 required-scope compiler error；未记录 no-full-material-parity claim boundary |
| `AAN-11.9` | Registry Projection Gate | 将 AAN 的 scoped material evidence 投影给 Phase12 / EBench registry | clean `passed` 只能在 `missing_material_refs: []`、`missing_textures: []` 或 registry 可表示 scoped background warnings 时输出；否则输出 `ready_with_scoped_warnings` / blocked / re-export request；不把 waiver 伪装成 clean pass | Phase12 raw scanner 把 inactive residue 或 background warning 当成整包 no-MDL 触发器；clean pass 中隐藏 required material 缺口；下游需要手写 USD/MDL/texture patch |

`AAN-11.9` 的实现经验来自 soap-to-dish S2D-12：如果 consumer registry 不能把
background-only warning 表示为非阻塞状态，就不要把 warning 伪装成 clean `passed`。正确做法是
生成一个 native-MDL clean re-export，移除 raw background missing refs 或 inactive binary
residue，同时通过 approved runtime module 记录 `gltf/pbr.mdl` 这类 Isaac 自带模块。只有
Phase12-facing `missing_material_refs: []` 和 `missing_textures: []` 成立后，才允许下游
Phase13 compile 进入静态候选。

AAN-11 的 hard block：

- required material 的 helper MDL、internal texture 或 effective binding 无法解析；
- package 内残留 `/cpfs/...`、`omniverse://`、`http(s)://`、未授权绝对路径或 package escape；
- required material runtime compiler 报错、shader node 创建失败、missing texture 或红色/error fallback；
- 缺失依赖无法归因到 background/unbound scope，或 task contract 缺失导致无法判断是否影响
  required surface；
- rewrite 无 hash/provenance，或需要修改 source tree 才能运行；
- 下游 EBench/LabUtopia 仍然需要手工修 USD/MDL/texture path。

AAN-11 可 temporary waiver 的范围：

- unbound/background-only material warning，且 required material runtime evidence clean；
- registry residue 或 binary residual string，且 pxr-level active traversal、package closure 和
  runtime log 都证明它不是 active dependency；
- 非语义关键的 clearcoat、anisotropy、procedural/noise 等高级材质差异，source asset 已保留，
  fallback provenance 已记录；
- explicit PreviewSurface fallback 仅用于兼容，manifest 禁止 full material parity；
- renderer/plugin/deprecation warning 与材质解析无关，cold load/render/step/reset 仍通过。

AAN-11 必须继续遵守 Isaac 4.1 环境只读策略：使用 `./scripts/isaac_python.sh` 和既有
`conda isaac41`/`ISAAC_SIM_ROOT`，禁止 `pip install`、`conda install/update/remove` 或修改
...
- 先跑 MDL-preserving dependency relevance + runtime visual candidate；`O.mdl` 这类
  registry residue 不应触发 fallback；
- 如果 active 官方 MDL/texture 依赖可取得，走 source-preserving mirror；
- 如果 required-scope 官方依赖不可取得、Isaac 4.1 无法 clean compile required material、
  或 required surface 视觉上 fallback，才走正式 no-MDL / OmniPBR /
...
- 如果当前 Phase12 schema 不能表达 scoped background warning，就必须 re-export 或重打包为
  clean registry candidate；soap-to-dish S2D-12 的参考 handoff 是
...
| Numbered acceptance reviewer | AAN 编号计划与验收标准 | 使用 `AAN-00` 到 `AAN-09` 管理 USD -> EBench MVP；`AAN-10` 是 MJCF scout，`AAN-11` 是材料 runtime follow-up，`AAN-12+` 再进入批量准入和多 profile 扩展 |
...
第三轮针对 material runtime closure follow-up：

| Reviewer | Focus | 采纳结论 |
|---|---|---|
| Architecture planner | follow-up 编号和模块边界 | 使用 `AAN-11 Material Runtime Closure`，不重开 `AAN-04` / `AAN-06`；新增 `mdl_runtime_closure.py` 边界，负责 MDL transitive graph、rewrite provenance 和 runtime material evidence |
| Feature designer / asset validator | MDL import、texture、resolution、rewrite policy | 先 mirror 后 rewrite；递归扫描 `import`、`using`、非空 `texture_2d`；每条 dep 必须分类为 `mirrored/native_runtime_module/waived/blocked`；KooPbr 旧修复不能泛化 |
| Isaac runtime reviewer | compiler log、render metric、environment policy | AAN-06 smoke 不等于 material runtime closure；AAN-11 解析 MDLC/failed shader/missing texture，并增加 front/door-facing 或透明物体视角；继续只读使用既有 Isaac 4.1 环境 |
| Docs / consumer handoff reviewer | PM 口径和下游责任边界 | Phase 1 ready 仍成立为 package/runtime/benchmark ready；full material runtime closure 是 AAN-11 follow-up；下游不能本地手工修 USD/MDL/texture |
| Soap-to-dish follow-up reviewers | MDL-first route policy | 对 `O.mdl`/JPG 缺口不再直接选择 no-MDL；先做 active dependency、binding scope、runtime compiler 和多视角视觉判断。no-MDL 是 compatibility fallback，不是默认保真路线。 |
...
- material runtime closure 作为 `AAN-11` 独立 follow-up；它补齐 MDL helper、MDL 内部
  texture、binding scope、compiler log 和多视角 render evidence，但不声明 full material
  parity。
- material fallback 采用 MDL-first route decision：静态缺失、registry residue 或背景材质
