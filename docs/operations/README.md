# Operations Documents

Runbooks, CLI usage, build instructions, troubleshooting, and agent collaboration notes.

## Setup And CLI

- [Project setup](../setup.md)
- [Isaac Python wrapper](isaac-python-wrapper.md)
- [Tools overview](tools.md)
- [Research asset layout](research-asset-layout.md)
- [InternNav official evidence runbook](internnav-official-evidence-runbook.md)
- [Inspect material](inspect-material.md)
- [Orbit CLI camera pose](orbit-cli-camera-pose.md)

## Conversion Workflows

- [Asset Application Normalizer consumer handoff](asset-application-normalizer-consumer-handoff.md)
- [Canonical task-object facade](build-canonical-task-object-facade.md)
- [Articulated device admission and requalification](articulated-device-admission-requalification.md)
- [Normalize an articulated appliance](normalize-articulated-appliance.md)
- [Simple-SDF and multi-liquid CLI](simple-sdf-multi-liquid.md)
- [Funnel and 15 mL tube small-v2 PBD parameter card](funnel-tube15-small-particle-pbd.md)
- [Threaded 15 mL tube and funnel exact-source PBD parameter card](wangshuai-threaded-tube-funnel-pbd.md)
- [Workspace profiling runbook](workspace-profiling.md)
- [GLB export](glb-export.md)
- [GLB export zh](glb-export-zh.md)
- [Export MDL materials](export-mdl-materials.md)
- [Export MDL materials usage](export-mdl-materials-usage.md)
- [Export MDL materials troubleshooting](export-mdl-materials-troubleshooting.md)

## Mesh

- [Mesh CLI usage](mesh-cli-usage.md)
- [Mesh face counting](mesh-face-counting.md)
- [Mesh performance and limits](mesh-performance-and-limits.md)
- [Native meshqem usage](native-meshqem-usage.md)
- [Native meshqem Python/C++ tutorial](native-meshqem-python-calls-cpp.md)

## Thumbnails

- [Local Isaac single-asset rendering](local-rendering.md)
- [Thumbnails overview](thumbnails.md)
- [Thumbnails environment](thumbnails-environment.md)
- [Thumbnails usage](thumbnails-usage.md)
- [Thumbnails troubleshooting](thumbnails-troubleshooting.md)

## Troubleshooting

- [Troubleshooting index](troubleshooting.md)
- [No materials preview](troubleshooting-no-materials-preview.md)
- [no-MDL failure analysis](troubleshooting-no-mdl-failure-analysis.md)
- [Orbit camera capture pitfalls](troubleshooting-orbit-camera-capture-pitfalls.md)

## Agent Operations

- [Agent team playbook](agent-team-playbook.md)
- [Codex agent playbook](codex-agent-playbook.md)
