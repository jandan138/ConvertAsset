...
- **Fix:** Warnings originate from the source USD and do not block rendering. Documented them in `docs/records/orbit-camera-headless.md`; investigate separately if image artifacts become noticeable.
...
5. Retain the command history in `docs/records/orbit-camera-headless.md` for reproducibility.
