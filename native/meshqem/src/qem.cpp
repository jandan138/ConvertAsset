...
static bool solve3(const double A[9], const double b[3], double x[3]){
    ...
    for(int i=0;i<3;++i){
        ...
        if(pv<1e-12) return false; if(piv!=i) for(int c=0;c<4;++c) std::swap(M[i][c],M[piv][c]);
...
bool qem_simplify(Mesh& mesh, const SimplifyOptions& opt, SimplifyReport& rep){
    ...
    auto push_edge = [&](int u,int v){ if(u==v) return; if(u>v) std::swap(u,v); if(!adj[u].count(v)) return; 
    ...
    while(faces_cur>target && !heap.empty() && collapsed<max_collapses){
        ...
        // new position: midpoint (simple, robust)，可后续替换为求解位置并回写
