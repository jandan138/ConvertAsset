...
| Gemma4 local multimodal checkpoint | Primary VLM for GRScenes stress and clean-pool grounding probes. | Local checkpoint path records an Unsloth quantized Gemma4 E4B-style model. Local README metadata states `base_model: google/gemma-4-E4B-it` and Apache-2.0 license metadata, but the exact local checkpoint must still be matched to a public model ID/commit before final claim. | Base model license source is identified; exact local quantized checkpoint provenance is not final until matched. | `stress_predictions.jsonl.metadata.json`, clean-pool metadata files, `COMPUTE_RUNTIME_SUMMARY_DRAFT.md`. | Replace local path with anonymized public model ID, commit/hash, license, model size, quantization settings, and decoding settings in appendix/checklist. |
| Qwen2.5-VL-7B-Instruct | Secondary VLM diagnostic exposing coordinate-contract sensitivity. | Local README metadata for `Qwen/Qwen2.5-VL-7B-Instruct` reports Apache-2.0 and links Qwen papers; the public model-card URL is recorded in the readiness audit. | Apache-2.0 per checked model card/local README metadata. | `stress_expanded30_probes/*qwen25*metadata.json`, clean-pool metadata files, `COMPUTE_RUNTIME_SUMMARY_DRAFT.md`. | Add public model ID, license, local checkpoint hash/commit if available, and decoding settings. |
...
videos after anonymization. Use `FINAL_SUBMISSION_PACKET_CHECKLIST.md` as the
staging checklist. Do not upload raw source scene trees, full scratch USD
trees, InternNav raw frame directories, LMDBs, local model checkpoints, or
absolute `/cpfs/...` paths.
...
3. Confirm the software/runtime versions summarized in
