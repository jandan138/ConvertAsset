...
distribution. The added InternNav/DualVLN~\cite{Wei2026Ground} smoke run is a
one-episode protocol check: it shows that paired original/noMDL USDs can produce
real embodied metrics and a trajectory-shift failure case, but both conditions
failed the episode and the sample size is too small for success-rate claims. The
current pipeline also depends on Isaac Sim and USD assets whose licenses,
material graphs, and renderer settings may differ across datasets, so any
general guidelines should be validated on a larger and more heterogeneous scene
corpus.
