...
| Responsible NLP checklist weak answers | `OPENREVIEW_RESPONSIBLE_NLP_CHECKLIST.md` now provides copy-ready answers with current PDF section/page anchors and remaining human gates; `check_openreview_checklist.py` rejects missing questions, placeholders, bare yes/no/N/A answers, missing policy URLs, missing anchors, and missing AI-assistance wording. | Lower risk; final OpenReview copy, target-call wording, and author/legal confirmation for optional media still required |
