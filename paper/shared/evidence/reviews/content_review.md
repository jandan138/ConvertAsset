...
- **No figure numbers for missing figures:** The paper references Fig. 1 (render pairs), Fig. 2 (image quality), Fig. 3 (feature similarity), and Fig. 4 (t-SNE). Verify that all figure files exist and are correctly generated. The PDF paths point to `paper/shared/figures/` which is a relative path from the writing directory.
