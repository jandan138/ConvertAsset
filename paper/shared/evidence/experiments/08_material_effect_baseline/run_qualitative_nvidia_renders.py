#!/usr/bin/env python3
"""Run selected NVIDIA qualitative render commands from the manifest."""
...
DEFAULT_LOG_ROOT = RAW_DIR / "qualitative_render_logs"
DEFAULT_OUTPUT = RAW_DIR / "qualitative_nvidia_render_run_manifest.json"
...
def _safe_sample_id(sample_id: Any) -> str:
    return str(sample_id or "unknown").replace("/", "_").replace(" ", "_")


def _nvidia_records(manifest: dict[str, Any]) -> list[dict[str, Any]]:
    return [record for record in manifest.get("records", []) if record.get("condition") == NVIDIA_CONDITION]


def run_qualitative_nvidia_renders(
    ...
    *,
    ...
    failed = 0
    ready = 0
    for record in _nvidia_records(manifest):
        sample_id = str(record.get("sample_id") or "")
        image_path = Path(str((record.get("image") or {}).get("path") or ""))
        stdout_path = log_root / f"{_safe_sample_id(sample_id)}.stdout.txt"
        stderr_path = log_root / f"{_safe_sample_id(sample_id)}.stderr.txt"
    ...
    return {
        "schema_version": 1,
        "status": "material_effect_qualitative_nvidia_render_run",
        "generated_at_utc": _utc_now(),
        "generated_by": "paper/shared/evidence/experiments/08_material_effect_baseline/run_qualitative_nvidia_renders.py",
        "summary": {
            "nvidia_record_count": len(_nvidia_records(manifest)),
            "attempted_count": attempted,
            "skipped_existing_count": skipped,
            "failed_count": failed,
            "ready_output_count": ready,
            "force": bool(force),
            "all_outputs_ready": ready == len(_nvidia_records(manifest)) and failed == 0,
        },
        "records": records,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_qualitative_nvidia_renders(
        manifest,
