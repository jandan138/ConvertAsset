...
def write_risk_tex(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        lines.append(
            " & ".join(
                [
                    _latex_escape(_tex_display(row["effect"])),
                    _latex_escape(_tex_display(row["qualitative_status"])),
                    _latex_escape(_tex_display(row["relative_interpretation"])),
                    _latex_escape(_tex_display(row["claim_allowed"])),
                ]
            )
            + " \\\\"
        )
