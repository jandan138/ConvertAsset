...
TEX_DISPLAY = {
    ...
    "selected_panel_ready": "selected qualitative ready",
    "supplemental_visual_fail": "supplemental visual/PXR fail",
    "static_only": "static only",
    "selected_nvidia_failure_convertasset_target_retained": (
        "selected NVIDIA failure; ConvertAsset target retained"
    ),
    "both_converted_conditions_risky_static_gate_insufficient": (
        "both conversions lose checker evidence"
    ),
    "bounded_three_condition_static_and_qualitative_evidence": (
        "bounded three-condition evidence"
    ),
    "static_evidence_only": "static evidence only",
    "selected_nvidia_failure_case": "selected NVIDIA failure case",
    "diagnostic_limitation_case": "diagnostic limitation case",
    "bounded_static_and_selected_qualitative": "bounded static + selected qualitative",
    "static_readiness_only": "static readiness only",
}
...
def _tex_display(value: Any) -> str:
    return TEX_DISPLAY.get(str(value), str(value))
...
def write_risk_tex(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    covered = [
        row
        for row in rows
        if row.get("claim_allowed") == "bounded_static_and_selected_qualitative"
    ]
    display_rows: list[dict[str, str]] = []
    if covered:
        display_rows.append(
            {
                "effect": ", ".join(_tex_display(str(row["effect"])) for row in covered),
                "evidence": "selected qualitative ready; bounded three-condition evidence",
                "claim": "bounded static and selected qualitative",
            }
        )
    for row in rows:
        if row.get("claim_allowed") == "bounded_static_and_selected_qualitative":
        ...
        display_rows.append(
            {
                "effect": _tex_display(str(row["effect"])),
                "evidence": (
                    f"{_tex_display(row['qualitative_status'])}; "
                    f"{_tex_display(row['relative_interpretation'])}"
                ),
                "claim": _tex_display(str(row["claim_allowed"])),
            }
        )
    ...
    lines = [
        ...
        "\\toprule",
        "Effect & Evidence / interpretation & Claim allowed \\\\",
        "\\midrule",
    ]
