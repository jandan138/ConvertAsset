...
14-pair zoom material-shift stress pool, Qwen2.5-VL shows answer and raw-point
degradation on converted renders. These results support the use of VLM probes
as diagnostics for material conversion, but not yet as final downstream
robustness evidence.
...
\begin{itemize}
    ...
    normal maps), MDL-to-UsdPreviewSurface conversion appears low-risk under
    the tested visual and feature metrics (PSNR $>$ 35\,dB, SSIM $>$ 0.99).
...
environments. The VLM results are also pilot diagnostics: the clean pool remains
below the final benchmark gate, and Qwen coordinate semantics are unresolved.
...
to a final benchmark with canonical model outputs and adding downstream
navigation or manipulation proxies where appropriate; (3)~investigating the
