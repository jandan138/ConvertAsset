...
specular coatings. The effect-level ConvertAsset-vs-NVIDIA matrix makes this
boundary explicit: the GRScenes-covered opacity/transparency, emission,
normal/bump, and displacement/height bins support bounded selected qualitative
claims, while supplemental clearcoat is a selected NVIDIA target-loss failure
case and procedural texture remains a preservation limitation for both
...
about 199\,MB. A repeated three-scene official KuJiaLe benchmark gives a more
conservative companion result: all 18 required fresh-process original/noMDL
runs succeed, but the aggregate warmed-ready intervals overlap
(13.95\,s [11.64, 16.28] for original MDL and 14.12\,s [12.31, 16.24] for
noMDL). The practical conclusion is therefore two-part: conversion does
not buy much steady-state rendering throughput for shallow single-object scenes,
it can substantially improve startup behavior in some large composed scenes,
and official navigation scenes should be benchmarked directly rather than
assumed to inherit that speedup.
...
\begin{itemize}
    ...
    individually, as conversion quality varies with material complexity and the
    current procedural supplemental case does not preserve checker evidence in
    either converted condition.
...
performance benchmarks still cover a limited set of scene families: four
single-object furniture scenes, one GRScenes commercial interior, and three
official KuJiaLe navigation scenes under a headless RTX path-tracing setup.
They should not yet be generalized to all larger, heavily instanced, or
materially diverse environments, and the NVIDIA official-scene performance
baseline remains unavailable until matching converted official scenes are
generated and smoke-gated. The VLM results are also claim-bounded: the clean pool remains
below the clean final gate, the stress set is target-centered and category
imbalanced, and Qwen coordinate semantics are unresolved. The material-effect
matrix is likewise a selected diagnostic, not a population-level failure-rate
estimate across all MDL material mechanisms.
