...
\paragraph{GRScenes VLM Pilot Protocol.}
...
diagnostic pools separate: a 15-pair clean pool in which the target remains
visible in both original and converted renders, and a 14-pair zoom-stress pool
that intentionally includes visible material, color, and lighting shifts. Two
local VLM backends, Gemma4 and Qwen2.5-VL, are prompted to return a target
category answer and a point location. We score category-answer matches and
point-in-projected-box hits under both raw-image and normalized-1000
coordinate interpretations, because early probes showed that coordinate
semantics are a major protocol variable.
