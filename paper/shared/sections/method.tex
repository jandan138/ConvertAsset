...
\paragraph{GRScenes VLM Grounding Protocol.}
For the vision-language grounding extension, we use the original
GRScenes~\cite{Wang2024GRUtopia} scene tree read-only and materialize
converted counterparts in a scratch workspace.
Target objects are localized from the scene USDs, projected into matched camera
views, and filtered by blind visual QA before model evaluation. We keep two
pools separate by claim boundary. The 15-pair clean pool includes only
visual-QA PASS pairs and remains a preservation pilot below the configured
20-pair clean final gate. The frozen expanded stress set contains 30
target-visible PASS/WARN zoom pairs across eight target categories; WARN pairs
are allowed only because the stress question intentionally includes visible
material, color, or lighting changes. This stress set is target-centered and
category-imbalanced, so we treat it as a controlled material-shift stress set
rather than a broad GRScenes distribution. Two local VLM backends, Gemma4 and
Qwen2.5-VL, are prompted to return a target category answer and a point
location. We score category-answer matches and point-in-projected-box hits
under both raw-image and normalized-1000 coordinate interpretations, because
early probes showed that coordinate semantics are a major protocol variable.
...
cosine similarity. Higher similarity indicates that the feature
representations are more invariant to the material change. We report
