...
| 缺 per-material-effect 分析 | 🔄 已启动 | 新增 `material_effect_baseline/effect_sample_manifest.json` 和 `material_effect_baseline_summary.csv`，把 expanded30 GRScenes stress pair 链到 MDL 文件和 effect 标签并输出 6 effect x 3 condition 表；当前覆盖 opacity/transparency、emission、normal/bump、displacement/height。已补 4-case original / ConvertAsset / NVIDIA selected qualitative still panel，12/12 条件图片 ready；clearcoat 和 procedural texture 已通过 supplemental wrapper 各补 1 个样本并生成 6/6 supplemental stills。静态 gate 上 ConvertAsset 两个都过、NVIDIA procedural 过、NVIDIA clearcoat 失败；但 clean-room 视觉审核和 PXR 诊断显示 supplemental panel overall `FAIL`，procedural 的两个 converted 条件都缺 checker/base-color texture preservation，需要实现/定位后再写视觉保存 claim。 |
...
5. **补材料效应归因**：已建立第一版 effect sample manifest、sample-level condition manifest、effect-grouped readiness 表、4-case 三路定性图、clearcoat/procedural supplemental wrapper、三路静态 gate manifest、supplemental render panel、clean-room visual review 和 PXR preservation diagnostic；NVIDIA clearcoat failure 已从静态失败推进到 target-missing + rendered near-black failure case。下一步是决定是否重跑 NVIDIA clearcoat、实现或承认 procedural checker baking 缺口，以及 paper figure/rebuttal 整合。
