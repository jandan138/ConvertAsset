...
| 6b InternNav / VL-LN navigation | ✅ prep scripts | ✅ input pair | ⬜ runtime blocked | ⬜ | ⬜ |
| 7 综合讨论 | — | — | — | — | ⬜ |
...
| 3 | VLM/下游评测 | ✅ expanded30 material-shift stress 已完成；clean-pool 仍是 15-pair pilot；InternNav 输入桥接已打通但 runtime 未跑 | `stress_vlm_run_manifest_expanded30.json` 的 manifest-internal stress gate 已打开，canonical Gemma4 `stress_predictions.jsonl` / `stress_score_summary.json` 已生成，Qwen2.5-VL expanded30 diagnostic 已生成；`internnav_vln_downstream/prep_manifest.json` 已生成 1-scene original/no-MDL VLN mini pair 并记录 `ready_for_internnav_runtime`；论文 claim 仍限 frozen 30-pair target-centered stress set，InternNav SR/SPL 只有在 InternUtopia、H1 robot assets 和 VLN checkpoint 补齐后才能加入 |
