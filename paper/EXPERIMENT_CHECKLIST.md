...
4. **补 NVIDIA baseline 或明确不可比边界**：baseline smoke、sample-level NVIDIA conversion、static condition manifest、selected covered-bin qualitative still panel 和 supplemental missing-bin render panel 已通过；下一步是人工视觉审查、视觉质量度量/失败案例写作，并写清 ConvertAsset 与官方工具在 composition-preserving、batch scratch conversion、evidence manifest 方面的不同目标。
5. **补材料效应归因**：已建立第一版 effect sample manifest、sample-level condition manifest、effect-grouped readiness 表、4-case 三路定性图、clearcoat/procedural supplemental wrapper、三路静态 gate manifest 和 supplemental render panel；NVIDIA clearcoat failure 已从静态失败推进到 rendered near-black failure candidate。下一步是 human review 和 paper figure/rebuttal 整合。
