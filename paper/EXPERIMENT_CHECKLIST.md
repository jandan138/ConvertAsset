...
| 缺 NVIDIA 官方 baseline | 🔄 已启动 | 已用 NVIDIA official `MDL_to_glTF.usd` fixture 跑通 `omni.kit.asset_converter` smoke，并对 selected pool 的 5 个 unique source scenes 跑完 `usd_to_usd_preview`：5/5 成功、外部输出约 4.9 GiB；`baseline_conversion_manifest.json` 现在确认 30/30 original/noMDL/NVIDIA 样本静态 gate 都过。已补 4-case selected 三路 still panel；supplemental clearcoat/procedural wrapper 三路静态 gate 已补，其中 NVIDIA clearcoat 输出 shader=0，成为失败案例候选；仍缺系统视觉质量 head-to-head 和 supplemental render panel。 |
| 缺 per-material-effect 分析 | 🔄 已启动 | 新增 `material_effect_baseline/effect_sample_manifest.json` 和 `material_effect_baseline_summary.csv`，把 expanded30 GRScenes stress pair 链到 MDL 文件和 effect 标签并输出 6 effect x 3 condition 表；当前覆盖 opacity/transparency、emission、normal/bump、displacement/height。已补 4-case original / ConvertAsset / NVIDIA selected qualitative still panel，12/12 条件图片 ready；clearcoat 和 procedural texture 已通过 `supplemental_conversion_manifest.json` 各补 1 个静态样本，ConvertAsset 两个都过，NVIDIA procedural 过、clearcoat 静态 gate 失败。 |
...
4. **补 NVIDIA baseline 或明确不可比边界**：baseline smoke、sample-level NVIDIA conversion、static condition manifest 和 selected covered-bin qualitative still panel 已通过；下一步是视觉质量度量/失败案例，并写清 ConvertAsset 与官方工具在 composition-preserving、batch scratch conversion、evidence manifest 方面的不同目标。
5. **补材料效应归因**：已建立第一版 effect sample manifest、sample-level condition manifest、effect-grouped readiness 表、4-case 三路定性图、clearcoat/procedural supplemental wrapper 和三路静态 gate manifest；下一步补 supplemental render panel，并把 clearcoat NVIDIA failure 从静态失败推进到 rendered evidence。
