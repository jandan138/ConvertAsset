...
5. **补材料效应归因**：已建立第一版 effect sample manifest、sample-level condition manifest、effect-grouped readiness 表、4-case 三路定性图、clearcoat/procedural supplemental wrapper、三路静态 gate manifest、supplemental render panel 和 clean-room visual review；NVIDIA clearcoat failure 已从静态失败推进到 rendered near-black/missing-target failure case。下一步是 clearcoat retake、procedural checker 丢失定位，以及 paper figure/rebuttal 整合。
