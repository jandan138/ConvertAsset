...
| 缺 NVIDIA 官方 baseline | 🔄 已启动 | 已用 NVIDIA official `MDL_to_glTF.usd` fixture 跑通 `omni.kit.asset_converter` smoke，并对 selected pool 的 5 个 unique source scenes 跑完 `usd_to_usd_preview`：5/5 成功、外部输出约 4.9 GiB；`baseline_conversion_manifest.json` 现在确认 30/30 original/noMDL/NVIDIA 样本静态 gate 都过。已补 4-case selected 三路 still panel；仍缺系统视觉质量 head-to-head、失败案例筛选和 supplemental wrapper conversion/render。 |
| 缺 per-material-effect 分析 | 🔄 已启动 | 新增 `material_effect_baseline/effect_sample_manifest.json` 和 `material_effect_baseline_summary.csv`，把 expanded30 GRScenes stress pair 链到 MDL 文件和 effect 标签并输出 6 effect x 3 condition 表；当前覆盖 opacity/transparency、emission、normal/bump、displacement/height。已补 4-case original / ConvertAsset / NVIDIA selected qualitative still panel，12/12 条件图片 ready；clearcoat 和 procedural texture 仍是显式 0-sample 行，但 `supplemental_effect_candidate_manifest.json` 已找到本地官方/样本候选，下一步是 author bound wrapper scene 后再跑三路 baseline。 |
...
5. **补材料效应归因**：已建立第一版 effect sample manifest、sample-level condition manifest、effect-grouped readiness 表、4-case 三路定性图和 clearcoat/procedural supplemental candidate manifest；下一步 author wrapper stages、跑 supplemental 三路 baseline，并把失败案例从 rendered evidence 中筛出来。
