...
| 4 | 图表和结论 | 🔄 expanded30 stress 表、VLM qualitative figure、material-effect risk matrix 和 material-effect qualitative/failure figures 已接入；trade-off 结论仍需全文收束 | 质量图、VLM 表、失败案例/定性图、trade-off 结论全部进入 `paper/shared/figures/`、`paper/shared/tables/` 和 `results_manifest.yaml`；当前 expanded30 stress 表可作为 frozen stress-set evidence，material-effect risk matrix 可作为 reviewer-facing claim-boundary evidence，clean-pool 15-pair 表、旧 zoom-stress 表、coordinate ablation 和 failure taxonomy 仍是 pilot/protocol diagnostic |
...
| 缺 NVIDIA 官方 baseline | 🔄 已启动 | 已用 NVIDIA official `MDL_to_glTF.usd` fixture 跑通 `omni.kit.asset_converter` smoke，并对 selected pool 的 5 个 unique source scenes 跑完 `usd_to_usd_preview`：5/5 成功、外部输出约 4.9 GiB；`baseline_conversion_manifest.json` 现在确认 30/30 original/noMDL/NVIDIA 样本静态 gate 都过。已补 4-case selected 三路 still panel；supplemental clearcoat/procedural wrapper 三路静态 gate 与三路 render panel 已补，其中 NVIDIA clearcoat 输出 shader=0、PXR 诊断缺 `/World/ClearcoatTarget`、渲染 near-black/缺目标，成为 rendered failure case；clean-room 视觉审核已完成但 overall `FAIL`，所以当前仍缺系统视觉质量 head-to-head，不能写成成功三方对比。 |
| 缺 per-material-effect 分析 | 🔄 已启动 | 新增 `material_effect_baseline/effect_sample_manifest.json`、`material_effect_baseline_summary.csv` 和 `material_effect_risk_matrix.csv`，把 expanded30 GRScenes stress pair 链到 MDL 文件和 effect 标签并输出 6 effect x 3 condition 表与 6-row claim-boundary 风险矩阵；当前覆盖 opacity/transparency、emission、normal/bump、displacement/height，并为 clearcoat/procedural texture 各补 1 个 supplemental 官方/sample wrapper。4-case original / ConvertAsset / NVIDIA selected qualitative still panel 已 ready；supplemental 6/6 stills 已 ready，但 clean-room 视觉审核和 PXR 诊断显示 supplemental panel overall `FAIL`：NVIDIA clearcoat 缺目标，procedural 的两个 converted 条件都缺 checker/base-color texture preservation。正文已接入 risk matrix 和 qualitative/failure figures；当前能写 bounded covered-bin evidence、selected NVIDIA clearcoat failure、procedural limitation，不能写 all-effects 成功视觉保存 claim。 |
...
| large-scene performance 太窄 | 🔄 部分修复 | 已补官方 KuJiaLe / InteriorAgent 0031/0036/0066 三场景 original/noMDL fresh-process load-render benchmark：18/18 required runs 成功，ready-time/FPS/GPU memory 有 bootstrap CI 和 failure counts。它修复了“只有单场景无多重复”的一部分，但不是 NVIDIA 官方场景三方对比，也不是所有 GRScenes/InteriorNav 分布。 |
...
4. **补 NVIDIA baseline 或明确不可比边界**：baseline smoke、sample-level NVIDIA conversion、static condition manifest、selected covered-bin qualitative still panel、supplemental missing-bin render panel、clean-room visual review、PXR diagnostic 和正文 risk-matrix 写作已通过入账；下一步是补视觉质量度量，或写清 ConvertAsset 与官方工具在 composition-preserving、batch scratch conversion、evidence manifest 方面的不同目标。
5. **补材料效应归因**：已建立第一版 effect sample manifest、sample-level condition manifest、effect-grouped readiness 表、4-case 三路定性图、clearcoat/procedural supplemental wrapper、三路静态 gate manifest、supplemental render panel、clean-room visual review、PXR preservation diagnostic、effect-level risk matrix，并已接入 `Experiments`/`Discussion`/`Conclusion`/ACL `Limitations`。NVIDIA clearcoat failure 已从静态失败推进到 target-missing + rendered near-black failure case，procedural texture 已明确为 checker preservation limitation。下一步是决定是否重跑 NVIDIA clearcoat、实现或承认 procedural checker baking 缺口，并准备 rebuttal 专用叙述。
