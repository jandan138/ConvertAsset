# === Safe Orbit Capture (no touching your original camera ops) ===
...
# - Waits a few frames each step to ensure refresh, captures to <usd_dir>/nomdl_images
...
def _bbox_center_and_size(stage, path):
    ...
    if rng.IsEmpty(): raise RuntimeError(f"Empty bbox for prim: {path}")
