...
def _overlay_text(retargets: list[dict[str, Any]], prefixes: dict[str, str]) -> str:
    lines = [
        "#usda 1.0",
        "(",
    ...
    for item in retargets:
        ...
        mount_prim = source_prim
        for raw_prefix, mount_prefix in prefixes.items():
            if source_prim == raw_prefix or source_prim.startswith(raw_prefix + "/"):
                mount_prim = mount_prefix + source_prim[len(raw_prefix):]
                break
        node = tree
...
def build_consumer_facade(
    ...
    *,
    ...
    overlay_path.write_text(_overlay_text(retargets, prefixes), encoding="utf-8")
    facade_path = out_dir / "facade.usda"
    facade_path.write_text(
        _facade_text(source_usd, out_dir, mounts, consumer_scope, overlay_path.name),
        encoding="utf-8",
    )
    provenance = {
